...
task('load-props', ["install-npm-depends"], function() {
	...
	properties = JSON.parse(fs.readFileSync('package.json'));
	...
	properties.siteName = "{{ app_name }}";
	properties.siteLocation = "/var/www/{{ app_name }}/";
	properties.state = "live";
	
	// Build some paths from properties for use later on
	versionedPath = "/var/www/{{ app_name }}/live/.versions/" +
